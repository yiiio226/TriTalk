...
| 端点                   | 方法 | 描述                                   |
| ---------------------- | ---- | -------------------------------------- |
| `/subscription/status` | GET  | 获取当前用户订阅状态 (需认证)          |
| `/webhook/revenuecat`  | POST | RevenueCat Webhook 接收端点 (无需认证) |
...
│   │   ├── auth.ts        # 认证服务和中间件
│   │   ├── subscription.ts # RevenueCat 订阅管理服务 ✅
│   │   └── auth/
...
│   ├── security.md            # 安全文档
│   ├── azure_speech.md        # Azure Speech 发音评估指南
│   └── gcp_tts.md             # GCP Gemini TTS 集成指南 ✅
