...
import 'package:frontend/features/scenes/data/scene_service.dart';
...
class _OnboardingScreenState extends ConsumerState<OnboardingScreen> {
  ...
  Future<void> _completeOnboarding() async {
    ...
    try {
      ...
      // Step 2: Wait for scenes to be generated by database trigger
      await _waitForScenesGenerated();

      // Step 3: Refresh scene cache so HomeScreen has data
  ...
  /// Poll custom_scenarios table to confirm scenes have been generated
  /// by the database trigger before navigating to HomeScreen.
  Future<void> _waitForScenesGenerated({
    ...
    for (int i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (response.isNotEmpty) {
          debugPrint(
            '✅ [Onboarding] Scenes generated after ${(i + 1) * 500}ms',
          );
          return; // Scenes exist, exit polling
        }
      } catch (e) {
        debugPrint('⚠️ [Onboarding] Error checking scenes: $e');
      }
    ...
    debugPrint(
      '⚠️ [Onboarding] Scenes not found after max attempts, proceeding anyway',
    );
  }
