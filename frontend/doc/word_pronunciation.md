**中文** | [English](word_pronunciation_en.md)

# 单词发音播放方案

> 📋 **功能说明**: 在发音反馈页面，用户点击任意单词时，播放该单词的正确发音。

## 🎯 设计目标

- ✅ 低延迟：用户点击后快速响应
- ✅ 多语言：支持英语、中文、日语、韩语等多种语言
- ✅ 高质量：发音准确，与评估标准一致
- ✅ 成本可控：通过缓存减少 API 调用

---

## 📐 混合方案架构

```
用户点击单词
    ↓
[1. 检查本地缓存] ───(有)──→ 直接播放 ✅
    ↓ (无)
[2. 检查语言是否本地 TTS 支持]
    ↓
  ┌─(支持)────→ [3a] 本地 TTS 播放 ✅（零成本）
  │
  └─(不支持)──→ [3b] 请求云端 TTS → 播放 → 缓存
```

---

## 🔀 云端 TTS 服务选型

> ⚠️ **待决策**：以下两个服务均可使用，需根据实际需求选择。

### 方案 A: MiniMax TTS（已集成）

| 维度       | 说明                                |
| ---------- | ----------------------------------- |
| **状态**   | ✅ 已集成，`/tts/generate` 端点可用 |
| **多语言** | ⚠️ 主要优化中文，英语质量一般       |
| **延迟**   | 流式响应，首字节较快                |
| **优势**   | 无需额外开发，复用现有服务          |
| **劣势**   | 单价较高，非中文语言质量不确定      |

### 方案 B: Azure Speech TTS（需新增）

| 维度       | 说明                                         |
| ---------- | -------------------------------------------- |
| **状态**   | ⚠️ 需新增集成（已有 Azure 账号用于发音评估） |
| **多语言** | ✅ 140+ 语言，400+ 神经语音                  |
| **延迟**   | ~100-200ms                                   |
| **优势**   | 多语言质量稳定，与发音评估服务统一           |
| **劣势**   | 需额外开发                                   |

### 对比总结

| 维度       | MiniMax         | Azure                           |
| ---------- | --------------- | ------------------------------- |
| 开发成本   | 🟢 低（已集成） | 🟡 中（需新增）                 |
| 单价       | 🟡 ¥3.5/万字符  | 🟢 $15/百万字符 (≈¥1.08/万字符) |
| 中文质量   | 🟢 优秀         | 🟢 优秀                         |
| 英语质量   | 🟡 一般         | 🟢 优秀                         |
| 多语言支持 | 🟡 有限         | 🟢 140+ 语言                    |

---

## 💰 成本估算

### 基础假设

- 单词平均长度：7 字符
- 用户每天练习：100 个单词
- 月活跃用户：1000 DAU

### MiniMax TTS 成本

| 场景                     | 计算               | 成本 (RMB)    |
| ------------------------ | ------------------ | ------------- |
| 单价                     | ¥3.5 / 万字符      | -             |
| 每次单词 TTS             | 7 × ¥0.00035       | ¥0.00245      |
| 用户每天 100 词          | 100 × ¥0.00245     | ¥0.245        |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × ¥0.245 | **¥7,350/月** |
| 1000 DAU / 月 (90% 缓存) | ¥7,350 × 10%       | **¥735/月**   |

### Azure TTS 成本

| 场景                     | 计算                | 成本                  |
| ------------------------ | ------------------- | --------------------- |
| 单价                     | $15 / 百万字符      | -                     |
| 每次单词 TTS             | 7 × $0.000015       | $0.000105             |
| 用户每天 100 词          | 100 × $0.000105     | $0.0105               |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.0105 | **$315/月 (≈¥2,268)** |
| 1000 DAU / 月 (90% 缓存) | $315 × 10%          | **$31.5/月 (≈¥227)**  |

### OpenAI TTS 成本

#### TTS-1 (标准版)

| 场景                     | 计算                | 成本                  |
| ------------------------ | ------------------- | --------------------- |
| 单价                     | $15 / 百万字符      | -                     |
| 每次单词 TTS             | 7 × $0.000015       | $0.000105             |
| 用户每天 100 词          | 100 × $0.000105     | $0.0105               |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.0105 | **$315/月 (≈¥2,268)** |
| 1000 DAU / 月 (90% 缓存) | $315 × 10%          | **$31.5/月 (≈¥227)**  |

#### TTS-1-HD (高清版)

| 场景                     | 计算               | 成本                  |
| ------------------------ | ------------------ | --------------------- |
| 单价                     | $30 / 百万字符     | -                     |
| 每次单词 TTS             | 7 × $0.00003       | $0.00021              |
| 用户每天 100 词          | 100 × $0.00021     | $0.021                |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.021 | **$630/月 (≈¥4,536)** |
| 1000 DAU / 月 (90% 缓存) | $630 × 10%         | **$63/月 (≈¥454)**    |

### ElevenLabs 成本

> ⚠️ ElevenLabs 采用订阅制，按量折算约 $100-300/百万字符，以下按 $150/百万字符 估算。

| 场景                     | 计算               | 成本                     |
| ------------------------ | ------------------ | ------------------------ |
| 单价 (估算)              | ~$150 / 百万字符   | -                        |
| 每次单词 TTS             | 7 × $0.00015       | $0.00105                 |
| 用户每天 100 词          | 100 × $0.00105     | $0.105                   |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.105 | **$3,150/月 (≈¥22,680)** |
| 1000 DAU / 月 (90% 缓存) | $3,150 × 10%       | **$315/月 (≈¥2,268)**    |

### Google Cloud TTS 成本

#### Standard (标准版)

| 场景                     | 计算                | 成本               |
| ------------------------ | ------------------- | ------------------ |
| 单价                     | $4 / 百万字符       | -                  |
| 每次单词 TTS             | 7 × $0.000004       | $0.000028          |
| 用户每天 100 词          | 100 × $0.000028     | $0.0028            |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.0028 | **$84/月 (≈¥605)** |
| 1000 DAU / 月 (90% 缓存) | $84 × 10%           | **$8.4/月 (≈¥60)** |

> 💡 免费额度：WaveNet/Neural2 每月前 100 万字符免费，可覆盖约 143,000 次单词播放。

#### WaveNet / Neural2 (高质量版)

| 场景                     | 计算                | 成本                  |
| ------------------------ | ------------------- | --------------------- |
| 单价                     | $16 / 百万字符      | -                     |
| 每次单词 TTS             | 7 × $0.000016       | $0.000112             |
| 用户每天 100 词          | 100 × $0.000112     | $0.0112               |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.0112 | **$336/月 (≈¥2,419)** |
| 1000 DAU / 月 (90% 缓存) | $336 × 10%          | **$33.6/月 (≈¥242)**  |

### Amazon Polly 成本

#### Standard (标准版)

| 场景                     | 计算                | 成本               |
| ------------------------ | ------------------- | ------------------ |
| 单价                     | $4 / 百万字符       | -                  |
| 每次单词 TTS             | 7 × $0.000004       | $0.000028          |
| 用户每天 100 词          | 100 × $0.000028     | $0.0028            |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.0028 | **$84/月 (≈¥605)** |
| 1000 DAU / 月 (90% 缓存) | $84 × 10%           | **$8.4/月 (≈¥60)** |

#### Neural (神经网络版)

| 场景                     | 计算                | 成本                  |
| ------------------------ | ------------------- | --------------------- |
| 单价                     | $16 / 百万字符      | -                     |
| 每次单词 TTS             | 7 × $0.000016       | $0.000112             |
| 用户每天 100 词          | 100 × $0.000112     | $0.0112               |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.0112 | **$336/月 (≈¥2,419)** |
| 1000 DAU / 月 (90% 缓存) | $336 × 10%          | **$33.6/月 (≈¥242)**  |

> 💡 免费额度：Neural 引擎首年每月 100 万字符免费。

#### Generative (生成式)

| 场景                     | 计算               | 成本                  |
| ------------------------ | ------------------ | --------------------- |
| 单价                     | $30 / 百万字符     | -                     |
| 每次单词 TTS             | 7 × $0.00003       | $0.00021              |
| 用户每天 100 词          | 100 × $0.00021     | $0.021                |
| 1000 DAU / 月 (无缓存)   | 1000 × 30 × $0.021 | **$630/月 (≈¥4,536)** |
| 1000 DAU / 月 (90% 缓存) | $630 × 10%         | **$63/月 (≈¥454)**    |

### 成本对比

| 服务                   | 单价 (百万字符) | 无缓存 (¥/月) | 90% 缓存 (¥/月) | 质量评价      |
| ---------------------- | --------------- | ------------- | --------------- | ------------- |
| Google/AWS Standard    | $4              | ¥605          | ¥60             | 🟡 机械       |
| Azure Neural           | $15             | ¥2,268        | ¥227            | 🟢 优秀       |
| OpenAI TTS-1           | $15             | ¥2,268        | ¥227            | 🟢 优秀       |
| Google WaveNet/Neural2 | $16             | ¥2,419        | ¥242            | 🟢 优秀       |
| AWS Polly Neural       | $16             | ¥2,419        | ¥242            | 🟢 优秀       |
| OpenAI TTS-1-HD        | $30             | ¥4,536        | ¥454            | 🟢 高清       |
| AWS Polly Generative   | $30             | ¥4,536        | ¥454            | 🟢 高清       |
| MiniMax                | ¥3.5/万 (~$49)  | ¥7,350        | ¥735            | 🟢 中文优秀   |
| ElevenLabs (~$150)     | ~$150           | ¥22,680       | ¥2,268          | 🟢 声音克隆强 |

> 💡 **缓存策略至关重要**：无论选择哪个服务，90% 缓存命中率可降低 90% 成本。
>
> 📊 **推荐选择**：
>
> - **性价比首选**：Google Cloud Standard 或 AWS Polly Standard（最便宜，但质量一般）
> - **质量与成本平衡**：Azure Neural / OpenAI TTS-1 / Google WaveNet（$15-16/百万字符）
> - **中文场景**：MiniMax（已集成，中文质量优秀）
> - **声音克隆需求**：ElevenLabs（最贵，但功能最强大）

---

## 📝 实现步骤清单

### Phase 1: 后端 API

- [ ] **1.1** 新增 `/tts/word` 端点（复用现有 TTS 服务或新增 Azure）
- [ ] **1.2** 支持多语言参数（language, voice）
- [ ] **1.3** 返回 MP3 音频数据（base64 或 binary）
- [ ] **1.4** 添加 API 测试用例

### Phase 2: 前端基础

- [ ] **2.1** 添加 `flutter_tts` 依赖
- [ ] **2.2** 创建 `WordTtsService` 服务类
- [ ] **2.3** 实现本地 TTS 播放功能
- [ ] **2.4** 实现语言支持检测（`getLanguages()`）

### Phase 3: 缓存层

- [ ] **3.1** 设计单词音频缓存结构
- [ ] **3.2** 实现内存缓存（LRU）
- [ ] **3.3** 实现磁盘缓存（持久化）
- [ ] **3.4** 缓存过期策略

### Phase 4: 混合策略

- [ ] **4.1** 实现缓存优先检查
- [ ] **4.2** 实现本地 TTS fallback
- [ ] **4.3** 实现云端 TTS 请求 + 后台缓存
- [ ] **4.4** 错误处理和重试机制

### Phase 5: UI 集成

- [ ] **5.1** 在 `VoiceFeedbackSheet` 中添加单词点击事件
- [ ] **5.2** 添加播放状态指示（loading/playing）
- [ ] **5.3** 添加播放动画效果
- [ ] **5.4** 处理连续快速点击

### Phase 6: 测试与优化

- [ ] **6.1** 单元测试
- [ ] **6.2** 集成测试
- [ ] **6.3** 性能测试（延迟、缓存命中率）
- [ ] **6.4** 多语言测试

---

## 🔧 技术细节

### 后端 API 设计

```
POST /tts/word
Content-Type: application/json

Request:
{
  "word": "pineapple",
  "language": "en-US",
  "voice": "..."  // 可选，服务端有默认值
}

Response:
{
  "audio_base64": "//uQxAAAAAANIAAAAAE...",
  "format": "mp3",      // 见下方说明
  "duration_ms": 850    // 可选
}
```

> ⚠️ **音频格式说明**：
>
> | 服务    | 默认格式 | 备注                                   |
> | ------- | -------- | -------------------------------------- |
> | MiniMax | MP3      | 现有配置 `audio_setting.format: "mp3"` |
> | Azure   | 可配置   | 支持 MP3、WAV、OGG 等，实现时确认      |
>
> 实现时根据实际选用的服务调整 `format` 字段。前端播放器需兼容对应格式。

### 多语言配置

```dart
class LanguageConfig {
  final String minimaxVoice;  // MiniMax voice ID
  final String azureVoice;    // Azure voice ID
  final String localTtsLocale;
  final String displayName;
}

const Map<String, LanguageConfig> languageConfigs = {
  'en-US': LanguageConfig(
    minimaxVoice: 'English_Trustworthy_Man',
    azureVoice: 'en-US-JennyNeural',
    localTtsLocale: 'en-US',
    displayName: 'English (US)',
  ),
  'en-GB': LanguageConfig(
    minimaxVoice: 'English_Trustworthy_Man',
    azureVoice: 'en-GB-SoniaNeural',
    localTtsLocale: 'en-GB',
    displayName: 'English (UK)',
  ),
  'zh-CN': LanguageConfig(
    minimaxVoice: 'female-tianmei',
    azureVoice: 'zh-CN-XiaoxiaoNeural',
    localTtsLocale: 'zh-CN',
    displayName: '中文（简体）',
  ),
  'ja-JP': LanguageConfig(
    minimaxVoice: 'Japanese_IntellectualFemale',
    azureVoice: 'ja-JP-NanamiNeural',
    localTtsLocale: 'ja-JP',
    displayName: '日本語',
  ),
  'ko-KR': LanguageConfig(
    minimaxVoice: 'Korean_IntellectualFemale',
    azureVoice: 'ko-KR-SunHiNeural',
    localTtsLocale: 'ko-KR',
    displayName: '한국어',
  ),
  // 扩展更多语言...
};
```

### 缓存结构

```
📁 app_documents/
└── 📁 word_tts_cache/
    └── 📁 {language}/
        ├── {encoded_word}.{format}
        └── ...
```

> 💡 缓存按语言分目录，不按用户分。同一单词的发音对所有用户一致。

#### 文件命名规则

由于不同语言可能包含特殊字符（中文、日语、韩语、撇号、连字符等），文件名需要编码处理：

| 方案              | 示例                                      | 优缺点                           |
| ----------------- | ----------------------------------------- | -------------------------------- |
| URL 编码          | `pineapple.mp3`, `%E4%BD%A0%E5%A5%BD.mp3` | ⚠️ 大小写问题，长度可能超限      |
| **Hash (MD5)** ✅ | `a1b2c3d4e5f6.mp3`                        | ✅ 长度固定，无特殊字符          |
| Base64            | `cGluZWFwcGxl.mp3`                        | ⚠️ 可能较长，含 `/` `+` 特殊字符 |

> 📝 **建议**：使用 Hash（MD5）方案，配合元数据文件支持调试。

#### 命名处理流程

```
原始单词 → 统一转小写 → MD5 Hash → 文件名
```

示例：
| 原始单词 | 转小写 | MD5 Hash | 文件名 |
|----------|--------|----------|--------|
| `Hello` | `hello` | `5d41402abc4b2a76...` | `5d41402abc4b2a76.mp3` |
| `hello` | `hello` | `5d41402abc4b2a76...` | `5d41402abc4b2a76.mp3` |
| `你好` | `你好` | `7eca689f0d3389d9...` | `7eca689f0d3389d9.mp3` |

> 💡 **大小写处理**：统一转小写后再 hash，确保 `Hello` 和 `hello` 命中同一缓存。

#### 元数据文件（调试用）

为便于调试，每个语言目录下维护一个 `_metadata.json` 文件：

```json
{
  "5d41402abc4b2a76": {
    "word": "hello",
    "format": "mp3",
    "createdAt": "2026-01-16T10:30:00Z",
    "source": "minimax"
  },
  "7eca689f0d3389d9": {
    "word": "你好",
    "format": "mp3",
    "createdAt": "2026-01-16T10:31:00Z",
    "source": "minimax"
  }
}
```

缓存目录结构：

```
📁 word_tts_cache/
└── 📁 en-US/
    ├── _metadata.json
    ├── 5d41402abc4b2a76.mp3
    └── ...
└── 📁 zh-CN/
    ├── _metadata.json
    ├── 7eca689f0d3389d9.mp3
    └── ...
```

#### 缓存查询规则

**查询时应忽略文件扩展名**，以支持格式变更：

```dart
import 'package:crypto/crypto.dart';
import 'dart:convert';

// 生成缓存 key（统一小写 + MD5）
String getCacheKey(String word) {
  final normalized = word.toLowerCase();
  final bytes = utf8.encode(normalized);
  final hash = md5.convert(bytes).toString();
  return hash.substring(0, 16);  // 取前 16 位足够
}

// 查询缓存时，只匹配文件名前缀
Future<String?> findCachedAudio(String word, String language) async {
  final cacheKey = getCacheKey(word);
  final cacheDir = Directory('$basePath/$language');

  if (!await cacheDir.exists()) return null;

  // 查找任意格式的缓存文件
  await for (final file in cacheDir.list()) {
    final fileName = path.basenameWithoutExtension(file.path);
    if (fileName == cacheKey) {
      return file.path;  // 找到缓存，无论格式
    }
  }
  return null;  // 未命中缓存
}
```

这样即使从 MP3 切换到 WAV，现有缓存仍然有效。

### WordTtsService 接口

```dart
abstract class WordTtsService {
  /// 播放单词发音
  /// 自动选择最佳策略（缓存 → 本地 TTS → 云端 TTS）
  Future<void> speakWord(String word, String language);

  /// 预加载单词列表（后台下载）
  Future<void> preloadWords(List<String> words, String language);

  /// 检查语言是否支持本地 TTS
  Future<bool> isLocalTtsAvailable(String language);

  /// 清除缓存
  Future<void> clearCache({String? language});
}
```

---

## 📚 相关资源

### MiniMax TTS

- [MiniMax API 文档](https://www.minimax.chat/document/guides/T2A)
- [现有集成代码](../../backend/src/services/minimax.ts)
- [现有 TTS 端点](../../backend/src/server.ts) - `/tts/generate`

### Azure Speech TTS

- [Azure Speech TTS 文档](https://learn.microsoft.com/azure/ai-services/speech-service/text-to-speech)
- [Azure 支持的语音列表](https://learn.microsoft.com/azure/ai-services/speech-service/language-support?tabs=tts)
- [Azure 定价](https://azure.microsoft.com/en-us/pricing/details/speech/)

### 前端

- [flutter_tts 包](https://pub.dev/packages/flutter_tts)
- [发音反馈组件](../lib/features/chat/presentation/widgets/voice_feedback_sheet.dart)
